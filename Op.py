...
for module, package in pip_modules.items():
    try:
        __import__(module)
    except ImportError:
        subprocess.run(
...
def get_proxies():
    try:
        proxies = requests.get('https://raw.githubusercontent.com/TheSpeedX/SOCKS-List/master/http.txt').text.splitlines()
        return proxies
    except:
        return []
...
║     ╚═╝  ╚═╝╚═╝     ╚═╝  ╚═╝╚══════╝           ║
║                                               ║
║         𝙏𝙝𝙚 𝙀𝙡𝙞𝙩𝙚 𝙏𝙤𝙤𝙡 𝙗𝙮 𝘼𝙍𝘼𝙎             ║
║         Telegram: @W4_M4                      ║
║         Channel : t.me/pytho2n                ║
╚═══════════════════════════════════════════════╝
'''
print(logo)
GE,BE,GI,BI=0,0,0,0
...
video_url = "https://raw.githubusercontent.com/aras52/aras88/main/Hi%C3%A7%20Ak%C4%B1llanm%C4%B1yorsun%20be%20kadir%20_%20E%C5%9EREF%20R%C3%9CYA%20ED%C4%B0T%20_%20GLXXMSTRIDER%20-%20DON_T%20STOP%20-%20Slowed(1080P_60FPS).mp4"


caption = (
    "<b>╭─────✧─────╮</b>\n"
    "<b>Ｄｅｖ:</b> @W4_M4\n"
    "<b>Ｃｈａｎｎｅｌ:</b> <a href='https://t.me/pytho1n'>ᴇɴᴛʀ</a>\n"
    "<b>╰─────✧─────╯</b>"
)
...
def info(email):
    ...
    try:
        ...
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.96 Safari/537.36",
...
def hso_gmail(email):
   ...
   #
    if 'password' in response:
        ...
        print(f'''
═════════════════════════════════════
 [ {F} True : {M}{GE}{Z} | False: {M}{BE}{X} | NO: {M}{BI}{C}{S} ] 
{F}═════════════════════════════════════
''')
    else:
        BE+=1
...
def HSO_to_examine(email):
    ...
    data = {
        'signed_body':'ef02f559b04e8d7cbe15fb8cf18e2b48fb686dafd056b7c9298c08f3e2007d43.{"_csrftoken":"dG4dEIkWvAWpIj1B2M2mutWtdO1LiPCK","adid":"b0382b46-1663-43a7-ba90-3949c43fd808","guid":"b0382b46-1663-43a7-ba90-3949c43fd808","device_id":"android-71a5d65f74b8fcbc","query":"'f'{email}''"}',

        'ig_sig_key_version':'4',
    }
    try:
        res = requests.post('https://i.instagram.com/api/v1/accounts/send_recovery_flow_email/',headers=headers,data=data,proxies=proxy).text
        #print(res)
